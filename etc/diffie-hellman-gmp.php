 ...
 *
 *  This implementation is in PHP, but requires the GMP multiprecision math extension to be enabled.
 */
...
function genDiffieHellmanMsg( $keylen )
{
    ...
    $prime = gmp_strval(gmp_nextprime(gmp_init($RandomStr,16)),16); 
...
function calcDiffieHellmanSecret( $gen, $exp, $mod, $message = null )
{
    ...
    $sec = gmp_powm($gen,$exp,$mod);
    $message = gmp_strval($sec,16);
