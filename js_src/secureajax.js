...
function bbSecureAjaxLayer() {
	...
	// Base URL for all the SecureAjax helper APIs.
	...
	function doLoadScpt( scriptname, id, reqScr, callback ) {
		// XML message to send to APIs (load script)
		var msg = "<loadScript name='" + scriptname + "'/>";
		
		// Make sure we have an id (get the next one if none passed in)
		if( id == null || id == "" ) {
			id = scriptDependencyCache.length + 1;
		}
		
		// Initially set the cache entry to false (always pre-set variables)
		scriptDependencyCache[id] = false;
		
		// Now, send the message to load the script
		doSendSecure( serverURL + "SecureAjaxAPIs.php", msg, function( doc ) {
			// If we get a response
			if( doc != null ) {
				// Get the script response (this also makes sure we got a good de-crypt)
				var scr = doc.getElementsByTagName( 'script' )[0];
				
				// Now, if the tag is there, and we have a proper type attribute,
				if( scr && scr.getAttribute( 'type' ) ) {
					// Get the type value
					var t = scr.attributes.getNamedItem( 'type' ).value;
					
					// If it's also correct (I like double checking)
					if( t.indexOf( 'text/javascript' ) > -1 ) {
						
						// Make sure the required scripts are loaded. 
						if( requiredScriptsLoaded( reqScr ) ) {
							
							// send the script to the callback
							callback( getTextNode( scr ) );
							// Indicate in the dependency cache that the script is loaded and executed
							scriptDependencyCache[id] = true;
							// and run any scripts that may be waiting on this one.
							checkForScriptsReady();
						} else {
							
							// Otherwise, is we're waiting on dependencies, 
							// set the cache to hold this entry, and execute it when able. 
							scriptDependencyCache[id] = {
								req:reqScr,
								text:getTextNode(scr),
								callback:callback
							};
						}
					}
	...
	//
	// Check in the script dependency cache for scripts that have all their dependencies met,
	// and can be run. Execute those scripts. Checks every 1/4 second for scripts that can be run.
	//
	function checkForScriptsReady() {
		
		// Check everything in the cache
		for( var i in scriptDependencyCache ) {
			// If this is a waiting script (i.e is hasn't been run yet)
			if( scriptDependencyCache[i] !== true && scriptDependencyCache[i] !== false ) {
				...
				if( requiredScriptsLoaded( scriptDependencyCache[i].req ) ) {
					// If they've been met, run them, and set the entry to true. 
					// (we assume any scripts that are dependent on this one are later in the queue)
					scriptDependencyCache[i].callback( scriptDependencyCache[i].text );
					scriptDependencyCache[i] = true;
				}else{
					// Else, if we're not ready, make sure we check again in 1/4 second.
					// TODO: move this out of the loop so we don't have a tone of timeout entries.
	...
	//
	//  Checks the status of prerequisites for a given script
	// to ensure they have all executed.
	//
	function requiredScriptsLoaded( req ) {
		
		// If we haven't been passed any prereqs, then simply indicate success.
		if( req === null || req == "" ) {
			return true;
		}
		
		// Otherwise, split the input into separate IDs
		var requiredScriptIds=req.split(' ');
		
		// And check each one for any that aren't ready
		var foundScript=false;
		for( var i = 0; i < requiredScriptIds.length; ++i ) {
			if( scriptDependencyCache[requiredScriptIds[i]] !== true ) {
				// Found one that's not ready
				foundScript = true;
				break;
			}
		}
		
		// Return true if all are ok, or false if any aren't ready.
		return( foundScript == false );
	};

	//
	// Indicates whether there are any scripts that still need to be loaded and/or executed.
	// 
	...
	//
	// Builds a list of all script IDs that are in the dependency cache.
	//
	...
	function doLoadImageFromServer( imgname, target ) {
		// Set message (load image) for the API call
		var msg = "<loadImage name='" + imgname + "'/>";
		
		// Make the API call
		doSendSecure( serverURL + "SecureAjaxAPIs.php", msg, function( doc ) {
			// Make sure we got a valid message back (properly deccrypted)
			if( doc != null ) {
				// If we did, get the image tag from the response.
				...
				// Finally, set the image node to the Data Url 
				document.getElementById( target ).src = "data:" + mimetype + ";base64," + base64rep;
			}
		});
	};
