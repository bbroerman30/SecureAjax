...
function AESEncryptCBC(plaintext, password, nBits) {
  var blockSize = 16;  // block size fixed at 16 bytes / 128 bits (Nb=4) for AES
  if (!(nBits==128 || nBits==192 || nBits==256)) return '';  // standard allows 128/192/256 bit keys
  plaintext = plaintext.encodeUTF8();
  password = password.encodeUTF8();

  // Copy the proper number of bytes into the cypher key.
  var key = new Array(nBits/8);
  for (var i=0; i< nBits/8; i++) {
    key[i] = isNaN(password.charCodeAt(i)) ? 0 : password.charCodeAt(i);
  }
  
  // Set up the initial IV (16 bytes of 0).
  var iv = new Array(16)
  for (var i=0; i< 16; i++) {
    iv[i] = 0;
  }
  ...
  for (var b=0; b<blockCount; b++) {
    var cypherBlock = new Array(16);

    // XOR plaintext block with iv block (cypher text of last block).
    for (var i=0; i<blockSize; i++) {  // -- xor plaintext with last block's ciphered text (iv) char-by-char --
      cypherBlock[i] = iv[i] ^ plaintext.charCodeAt(b*blockSize+i);
    }
    
    var outBlock = Cipher(cypherBlock, keySchedule);  // -- encrypt block --
    
    // Set up the IV for the next pass. Also set the outBlock.
    for (var i=0; i<blockSize; i++) {
      iv[i] = outBlock[i];
      outBlock[i] = String.fromCharCode(outBlock[i]);
    }
    
    // Save the ciphertext from this block.
    ciphertxt[b] = outBlock.join(''); 
  }
