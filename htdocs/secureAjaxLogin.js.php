...
function hex_sha1(w){return binb2hex(core_sha1(str2binb(w),w.length*chrsz))}
...
function toHexStr(cryptext){var chrsz="",v1;for(var i1=7;i1>=0;i1--){v1=(cryptext>>>(i1*4))&0xf;chrsz+=v1.toString(16);}return chrsz;}
function ROTR(n2,x2){return(x2>>>n2)|(x2<<(32-n2));}
function Sigma0(x3){return ROTR(2,x3)^ROTR(13,x3)^ROTR(22,x3);}
function Sigma1(x4){return ROTR(6,x4)^ROTR(11,x4)^ROTR(25,x4);}
function sigma0(x5){return ROTR(7,x5)^ROTR(18,x5)^(x5>>>3);}
function sigma1(x6){return ROTR(17,x6)^ROTR(19,x6)^(x6>>>10);}
function Ch(x7,y7,z7){return(x7&y7)^(~x7&z7);}
function Maj(x7,y7,z7){return(x7&y7)^(x7&z7)^(y7&z7);}
...
function hexSHA256(msg){msg=msg.encodeUTF8()+addCharCode();var msglen=getMsgLength(msg);var msgln2=msglen/4+2;var NumBlocks=Math.ceil(msgln2/16);var MsgBlocks=new Array(NumBlocks);for(var indx=0;indx<NumBlocks;indx++){MsgBlocks[indx]=new Array(16);for(var innridx=0;innridx<16;innridx++){MsgBlocks[indx][innridx]=(msg.charCodeAt(indx*64+innridx*4)<<24)|(msg.charCodeAt(indx*64+innridx*4+1)<<16)|(msg.charCodeAt(indx*64+innridx*4+2)<<8)|(msg.charCodeAt(indx*64+innridx*4+3))}}MsgBlocks[NumBlocks-1][14]=((msglen-1)*8)/Math.pow(2,32);MsgBlocks[NumBlocks-1][14]=Math.floor(MsgBlocks[NumBlocks-1][14]);MsgBlocks[NumBlocks-1][15]=((msglen-1)*8)&4294967295;var HashTable=InitHashTable.slice(0);var MsgSched=new Array(64);var wk1,wk2,wk3,wk4,wk5,wk6,wk7,wk8;for(var indx=0;indx<NumBlocks;indx++){for(var innridx=0;innridx<16;innridx++){MsgSched[innridx]=MsgBlocks[indx][innridx]}for(var innridx=16;innridx<64;innridx++){MsgSched[innridx]=(sigma1(MsgSched[innridx-2])+MsgSched[innridx-7]+sigma0(MsgSched[innridx-15])+MsgSched[innridx-16])&4294967295}wk1=HashTable[0];wk2=HashTable[1];wk3=HashTable[2];wk4=HashTable[3];wk5=HashTable[4];wk6=HashTable[5];wk7=HashTable[6];wk8=HashTable[7];for(var t=0;t<64;t++){var T1=wk8+Sigma1(wk5)+Ch(wk5,wk6,wk7)+Konstants[t]+MsgSched[t];var T2=Sigma0(wk1)+Maj(wk1,wk2,wk3);wk8=wk7;wk7=wk6;wk6=wk5;wk5=(wk4+T1)&4294967295;wk4=wk3;wk3=wk2;wk2=wk1;wk1=(T1+T2)&4294967295}HashTable[0]=(HashTable[0]+wk1)&4294967295;HashTable[1]=(HashTable[1]+wk2)&4294967295;HashTable[2]=(HashTable[2]+wk3)&4294967295;HashTable[3]=(HashTable[3]+wk4)&4294967295;HashTable[4]=(HashTable[4]+wk5)&4294967295;HashTable[5]=(HashTable[5]+wk6)&4294967295;HashTable[6]=(HashTable[6]+wk7)&4294967295;HashTable[7]=(HashTable[7]+wk8)&4294967295}return toHexStr(HashTable[0])+toHexStr(HashTable[1])+toHexStr(HashTable[2])+toHexStr(HashTable[3])+toHexStr(HashTable[4])+toHexStr(HashTable[5])+toHexStr(HashTable[6])+toHexStr(HashTable[7])}; 
...
<?php
    ...
    $keywords = array( "that","Cipher","SubBytes","Sbox","ShiftRows","MixColumns","AddRoundKey","ExtendedLoginKey2","ExtendedLoginKey3","ExtendedLoginKey4","docScrText",
                       ...
                       "toHexStr","ROTR","Sigma0","Sigma1","sigma0","sigma1","Maj","hash","msg","utf8encode","hexSHA256","v1","i1","n2","y7","z7","HashTable","NumBlocks","Konstants",
                       "MsgBlocks","indx","innridx","MsgSched","T1","T2","wk1","wk2","wk3","wk4","wk5","wk6","wk7","wk8","wk9","msglength","rounds","addCharCode","getMsgLength","InitHashTable");
    ...
    //
    $_SESSION["logincallback"] = "logincallback";//array_search("logincallback",$encodedSet);
