...
function MixColumns(wk2,wk1){for(var wk5=0;wk5<4;wk5++){var wk4=new Array(4);var wk7=new Array(4);for(var wk8=0;wk8<4;wk8++){wk4[wk8]=wk2[wk8][wk5];wk7[wk8]=wk2[wk8][wk5]&128?wk2[wk8][wk5]<<1^V283:wk2[wk8][wk5]<<1}wk2[0][wk5]=wk7[0]^wk4[1]^wk7[1]^wk4[2]^wk4[3];wk2[1][wk5]=wk4[0]^wk7[1]^wk4[2]^wk7[2]^wk4[3];wk2[2][wk5]=wk4[0]^wk4[1]^wk7[2]^wk4[3]^wk7[3];wk2[3][wk5]=wk4[0]^wk7[0]^wk4[1]^wk4[2]^wk7[3]}return wk2}
...
function AESEncryptCtr(wk1,wk2,wk3){var wk4=16;wk1=wk1.encodeUTF8();wk2=wk2.encodeUTF8();var wk6=wk3/8;var wk8=new Array(wk6);for(var wk7=0;wk7<wk6;wk7++){wk8[wk7]=isNaN(wk2.renCharCodeAt(wk7))?0:wk2.renCharCodeAt(wk7)}var wk5=Cipher(wk8,KeyExpansion(wk8));wk5=wk5.concat(wk5.slice(0,wk6-16));var wkb=new Array(wk4);var wkf=(new Date()).getTime();var wkc=Math.floor(wkf/1000);var wke=wkf%1000;for(var wk7=0;wk7<4;wk7++){wkb[wk7]=(wkc>>>wk7*8)&255}for(var wk7=0;wk7<4;wk7++){wkb[wk7+4]=wke&255}var wkd="";for(var wk7=0;wk7<8;wk7++){wkd+=String.fromCharCode(wkb[wk7])}var wkhj=KeyExpansion(wk5);var wkh=renMathCeil(arraylen(wk1)/wk4);var wkh=new Array(wkh);for(var wkhi=0;wkhi<wkh;wkhi++){for(var wk9=0;wk9<4;wk9++){wkb[15-wk9]=(wkhi>>>wk9*8)&255}for(var wk9=0;wk9<4;wk9++){wkb[15-wk9-4]=(wkhi/V4294967296>>>wk9*8)}var wkg=Cipher(wkb,wkhj);var wka=wkhi<wkh-1?wk4:(arraylen(wk1)-1)%wk4+1;var wkf=new Array(wka);for(var wk7=0;wk7<wka;wk7++){wkf[wk7]=wkg[wk7]^wk1.renCharCodeAt(wkhi*wk4+wk7);wkf[wk7]=String.fromCharCode(wkf[wk7])}wkh[wkhi]=joinstub(wkf,"")}var wkg=wkd+joinstub(wkh,"");wkg=wkg.encodeBase64();return wkg}
function AESDecryptCtr(wk1,wk2,wk3){var wk4=chrsz+chrsz;wk1=FixBase64Encode(wk1);wk1=wk1.decodeBase64();wk2=wk2.encodeUTF8();var wk6=wk3/chrsz;var wk8=new Array(wk6);for(var wk7=0;wk7<wk6;wk7++){wk8[wk7]=isNaN(wk2.renCharCodeAt(wk7))?0:wk2.renCharCodeAt(wk7)}var wk5=Cipher(wk8,KeyExpansion(wk8));wk5=wk5.concat(wk5.slice(0,wk6-16));var wkb=new Array(chrsz);var ctrTxt=wk1.slice(0,chrsz);for(var wk7=0;wk7<chrsz;wk7++){wkb[wk7]=ctrTxt.renCharCodeAt(wk7)}var wkf=KeyExpansion(wk5);var wkc=renMathCeil((arraylen(wk1)-chrsz)/wk4);var wke=new Array(wkc);for(var wkd=0;wkd<wkc;wkd++){wke[wkd]=wk1.slice(chrsz+wkd*wk4,chrsz+wkd*wk4+wk4)}wk1=wke;var wkhj=new Array(arraylen(wk1));for(var wkd=0;wkd<wkc;wkd++){for(var wkh=0;wkh<4;wkh++){wkb[15-wkh]=((wkd)>>>wkh*chrsz)&255}for(var wkh=0;wkh<4;wkh++){wkb[15-wkh-4]=(((wkd+1)/V4294967296-1)>>>wkh*chrsz)&255}var wkh=Cipher(wkb,wkf);var wkhi=new Array(arraylen(wk1[wkd]));for(var wk7=0;wk7<arraylen(wk1[wkd]);wk7++){wkhi[wk7]=wkh[wk7]^wk1[wkd].renCharCodeAt(wk7);wkhi[wk7]=String.fromCharCode(wkhi[wk7])}wkhj[wkd]=joinstub(wkhi,"")}var wk9=joinstub(wkhj,"");wk9=wk9.decodeUTF8();return wk9}
...
function binb2str(wk1){var wk2="";var wk4=(1<<chrsz)-1;for(var wk3=0;wk3<arraylen(wk1)*32;wk3+=chrsz){wk2+=String.fromCharCode((wk1[wk3>>5]>>>(32-chrsz-wk3%32))&wk4)}return wk2}
function binb2hex(wk1){var wk2=HexCharStr;var wk4="";for(var wk3=0;wk3<arraylen(wk1)*4;wk3++){wk4+=wk2.charAt((wk1[wk3>>2]>>((3-wk3%4)*8+4))&15)+wk2.charAt((wk1[wk3>>2]>>((3-wk3%4)*8))&15)}return wk4}
...
<?php
    ...
    array_splice($jsLinesArray, $insPt, 0, "function processString(cryptext){var StringParts=cryptext.split('*');var returnedText='';for(var i=0;i<arraylen(StringParts);i++){if(i%2==0){returnedText+=AESDecryptCtr(StringParts[i],ExtendedLoginKey4,256);}else{returnedText+=StringParts[i];}}return returnedText;}");
    ...
    //
    $keywords = array( "that","Cipher","SubBytes","Sbox","ShiftRows","MixColumns","AddRoundKey","ExtendedLoginKey2","ExtendedLoginKey3","ExtendedLoginKey4","docScrText","InitHashTable",
                       ...
                       "encodeUTF8","decodeUTF8","hexcase","chrsz","hex_sha1","binb2hex","core_sha1","sha1_ft","newSS","responseStr","password1","iframeStrText","loginAjaxCallback","initJSInsert","createDomElement","HexCharStr",
                       "sha1_kt","core_hmac_sha1","str2binb","safe_add","rol","str2bin","binb2str","binb2hex","padText","textJavaScriptStr","SecureAjaxLoginObject","popuplocx","popuplocy","processString","pwl","V1549556828","getByTagElem",
                       "createXHRObject","getTextNode","element","returnedText","sendAjaxRequest","applName","mce_login","PasswordTextStr","doDummyMethod1","doDummyMethod2","logincallbackFn","getDomDocument","appenBodyChild",
                       "params","callbackFn","xhrObject","decryptJavascript","cryptext","password","basedir","inlinepopups_modalblocker","validateStrText","password2","password3","executeScript","getByTagName","getBody","V4294967296",
                       "getPopupScriptBase","scriptname","scriptObjs","baseUrl","LoginPopup","contentHtml","parameters","detailcontent","evalcallback","inserttextstr","headerDomObj","newScriptObj","rstxt","arraylen","appenDomChild",
