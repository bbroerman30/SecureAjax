...
function doGetDHKey(){DHNegotiationInProgress=true;if(null==rsaFuncs||null==sha1Funcs){setTimeout('window.secureAjax.getSecretKeyDH();',100);return;}var randomExponent=rsaFuncs.makeRandomKey(16);var sharedSecretStr=rsaFuncs.getDHSecret(diffieHellman.msg,diffieHellman.mod,randomExponent);var messageToSign=rsaFuncs.getDHSecret(diffieHellman.gen,diffieHellman.mod,randomExponent);var signature=signMessage(messageToSign);var params="msg="+messageToSign+"("+signature+")";sendAjax('/dhkeyserver.php',params,function(stt,doc){if(null==doc){channelReadyCallbackFn(true);return;}if(doc.getElementsByTagName('success')[0]){sharedSecretKey=sha1Funcs.hexSHA1(sharedSecretStr);DHNegotiationInProgress=false;if(null!=channelReadyCallbackFn){channelReadyCallbackFn(true);}}else{alert("Error in setting up secure communication.");if(null!=channelReadyCallbackFn){channelReadyCallbackFn(false);}}}); };
